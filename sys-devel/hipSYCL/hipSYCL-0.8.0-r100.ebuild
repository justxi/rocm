...
#	SRC_URI="https://github.com/illuhad/hipSYCL/archive/v${PV}.tar.gz -> hipSYCL-${PV}.tar.gz"
	EGIT_REPO_URI="https://github.com/illuhad/hipSYCL"
	EGIT_BRANCH="master"
	EGIT_COMMIT="v0.8.0"
        inherit git-r3
...
REQUIRED_USE="nvidia? ( ^^ ( cuda9 cuda10 ) )"

# While the needed dependencies for CUDA are almost in Gentoo portage,
# the dependencies for ROCm are not completely stable
DEPEND="rocm? ( =sys-devel/hip-2.8.0[llvm-roc-backend,-hcc-backend] )
	rocm? ( =sys-devel/llvm-roc-2.8.0-r1[-debug] )
...
src_configure() {
	...
	if use rocm; then
		local mycmakeargs=(
			...
			-DCLANG_INCLUDE_PATH=/usr/lib/llvm/roc/lib/clang/10.0.0/include/
			-DROCM_PATH=/usr/
			-DHIPSYCL_CUDA_PATH="/opt/cuda/"
		)
	fi
