...
rocm? ( https://github.com/eigenteam/eigen-git-mirror/archive/d41dc4dd.tar.gz -> eigen-git-mirror-d41dc4dd.tar.gz )
...
# https://github.com/protocolbuffers/protobuf/archive/d0bfd522.tar.gz -> protobuf-d0bfd522.tar.gz
...
IUSE="asan atlas cuda rocm eigen +fbgemm ffmpeg gflags glog +gloo leveldb lmdb mkl mkldnn mpi namedtensor +nnpack numa +numpy +observers +openblas opencl opencv +openmp +python +qnnpack redis static test tools zeromq"
...
RDEPEND="
	...
	rocm? ( >=sys-devel/hip-3.5
			...
			>=sci-libs/miopen-3.5
			>=sci-libs/rocBLAS-3.5
			>=sci-libs/rocRAND-3.5
			>=sci-libs/hipSPARSE-3.5
			>=sci-libs/rocFFT-3.5
			>=dev-util/roctracer-3.5 )
...
src_prepare() {
	...
	if use rocm; then
		...
		${S}/tools/amd_build/build_amd.py
		eend $?
	fi
	...
	if use rocm; then
		sed -e "s,third_party/eigen,../eigen-git-mirror,g" -i ${S}/cmake/Dependencies.cmake || die
	fi
}
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DUSE_SYSTEM_EIGEN_INSTALL=$(usex rocm OFF ON)
